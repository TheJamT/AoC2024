...
fn main() -> Result<()> {
    ...
    let result: u32 = sequences
        ...
        .filter_map(|sequence| {
            ...
            split.sort_by(|a, b| {
                if let Some(a_set) = number_set.get(a) {
                    if let Some(b_set) = number_set.get(b) {
                        if a_set.after.contains(b) || b_set.before.contains(a) {
                            Ordering::Less
                        } else if a_set.before.contains(b) || b_set.after.contains(a) {
                            Ordering::Greater
                        } else {
                            Ordering::Equal
                        }
